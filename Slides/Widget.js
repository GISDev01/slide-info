...
define([
  ...
  var clazz = declare([BaseWidget], {
    ...
    onClose: function () {
      this.inherited(arguments);
      this._updateSlideInfoText(event);
      this._collapse();
    },
    ...
    _onSlideBarClicked: function (event) {
      ...
      if (target.slide) {
        this._updateSlideInfoText(target.slide.title.text);
        target.slide.applyTo(this.sceneView);
      }

    },

    _updateSlideInfoText: function (slideTitleText) {
      if (typeof(slideTitleText) !== "string") {
        console.log("Clearing slide info text.");
        document.getElementById("slideInfosContainer").innerHTML = "";
        return;
      }

      console.log("Getting Description from Config for Slide Title: " + slideTitleText);

      if (slideTitleText in this.config) {
        document.getElementById("slideInfosContainer").innerHTML = this.config[slideTitleText];
      }
      else {
        console.log("This slide title wasn't found in the config file. " +
          "It contains the following Slide Titles: " + Object.keys(this.config));

        // No data found in the config file for this Slide Title
        document.getElementById("slideInfosContainer").innerHTML = "";
      }
    }
  });
